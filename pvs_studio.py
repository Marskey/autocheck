    ...
    def __gen_plog(self):
        ...
        for parent, dirnames, filenames in os.walk("temp",  followlinks=True):
            for file in filenames:
                ...
                if (ret & 1) / 1 == 1:
                    printer.aprint("PVS-Studio error msg: FilesFail")
                if (ret & 2) / 2 == 1:
                    printer.aprint("PVS-Studio error msg: GeneralExeption")
                if (ret & 4) / 4 == 1:
                    printer.aprint("PVS-Studio error msg: IncorrectArguments")
                if (ret & 8) / 8 == 1:
                    printer.aprint("PVS-Studio error msg: FileNotFound")
                if (ret & 16) / 16 == 1:
                    printer.aprint("PVS-Studio error msg: IncorrectCfg")
                if (ret & 32) / 32 == 1:
                    printer.aprint("PVS-Studio error msg: InvalidSolution")
                if (ret & 64) / 64 == 1:
                    printer.aprint("PVS-Studio error msg: IncorrectExtension")
                if (ret & 128) / 128 == 1:
                    printer.aprint("PVS-Studio error msg: IncorrectLicense")
                if (ret & 256) / 256 == 1:
                    printer.aprint("PVS-Studio error msg: AnalysisDiff")
                if (ret & 512) / 512 == 1:
                    printer.aprint("PVS-Studio error msg: SuppressFail")
                if (ret & 1024) / 1024 == 1:
                    printer.aprint("PVS-Studio error msg: LicenseRenewal")
